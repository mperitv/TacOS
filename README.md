# TacOS 🌮

Welcome to **TacOS**, a retro-inspired WebOS interface featuring custom windows, apps, sounds and raining tacos!!!
This is my first WebOS ever!!
I hope you liked, feel free to give feedbacks!!
Also, I used AI to debugging my code and to learn how can I use audio files in TacOS.

## 👥 Credits

**Domino's Pizza** - Inspired an app name in my project (**DominOS** 🍕).
**Roblox** I used "Pizza" sound effect
**ParryGripp** I inspired "Raining Tacos - Parry Gripp & BooneBum"music video and "Taco Bot"
**ThomasOS** - For guidance


## 📂 Project Structure

```text
├── index.html              # Core application file (HTML, CSS, and JS)
└── tacos_background.PNG    # Desktop environment background asset


